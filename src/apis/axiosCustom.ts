...
export class Api {
  ...
  private _isExpiredTokenError(error: AxiosError | any) {
    return (
  ...
  private async refreshToken() {
    try {
      const response = await api.get(`${appUrls.authenURL}/refresh-token`, {
        headers: {
  ...
  private setupInterceptors() {
    this.instance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        if (!this._isExpiredTokenError(error)) return Promise.reject(error);
